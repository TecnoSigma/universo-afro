...
RSpec.describe Financial::PlanAdapter do
  describe '#create' do
    context 'when creates a new plan in payment gateway' do
      it 'returns gateway code of created plan' do
        ...
        allow_any_instance_of(described_class).to receive(:response) { result }

        gateway_result = described_class.new(action: :create).create!(name: name, reference: reference, price: price)
